...
const returnPage = pageParams.get("return") || "";
...
function configureManagementNavigation() {
  if (!requestedTrainingId || !["detail", "checkin"].includes(returnPage)) return;
  const eventQuery = encodeURIComponent(requestedTrainingId);
  $("managementNav").hidden = false;
  $("detailLink").href = `../training-detail.html?event=${eventQuery}&return=checkin`;
  if (returnPage === "detail") {
    $("backLink").textContent = "研修詳細へ戻る";
    $("backLink").href = `../training-detail.html?event=${eventQuery}`;
    $("detailLink").hidden = true;
  } else {
    $("backLink").href = "../checkin-select.html";
  }
}

async function loadTrainings() {
  $("training").replaceChildren(new Option("行事を読み込み中...", ""));
  ...
  try {
    ...
    $("training").replaceChildren(new Option("行事を取得できませんでした", ""));
    setStatus(`行事の取得に失敗しました: ${error.message || error}`, "error");
  } finally {
...
async function restoreIfCancelled(checkinId, checkinMethod) {
  try {
...
function showResult(title, body, completed = false) {
  ...
  if (completed) {
    if (!$("guestPanel").hidden) {
      ["guestName", "guestOrganization", "guestEmail", "guestPhone"].forEach((id) => $(id).value = "");
      $("guestStatus").textContent = "受付しました。続けて次の方を受付できます。";
      $("guestStatus").className = "status ok";
      $("guestName").focus();
    } else {
      ["memberNo", "companyName"].forEach((id) => $(id).value = "");
      $("companiesPanel").hidden = true;
      $("peoplePanel").hidden = true;
      $("companies").replaceChildren();
      $("people").replaceChildren();
      selectedCompany = null;
      setStatus("受付しました。絞り込み条件はそのままで、続けて次の会社を検索できます。", "ok");
      $("companyName").focus();
    }
  }
...
async function checkin(person, button) {
  ...
  try {
    await lookupTarget("PERSONAL", person.personalId);
    ...
    if (duplicateError(error) && await restoreIfCancelled(`${trainingId}:PERSONAL:${person.personalId}`, CHECKIN_METHODS.WEB_SEARCH)) {
      showResult("受付完了", `${selectedCompany.companyName} ${person.name} 様`, true);
      return;
    }
    showResult(
...
async function checkinCompany(button) {
  ...
  const trainingId = $("training").value;
  try {
    await lookupTarget("COMPANY", selectedCompany.memberNo);
    ...
    if (duplicateError(error) && await restoreIfCancelled(`${trainingId}:COMPANY:${selectedCompany.memberNo}`, CHECKIN_METHODS.WEB_SEARCH)) {
      showResult("受付完了", selectedCompany.companyName, true);
      return;
    }
    showResult(
...
async function initializePublicCheckin() {
  ...
  try {
    ...
    // 係員用検索受付は同じ端末で連続受付するため、過去の完了表示では画面を止めない。
