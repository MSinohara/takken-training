...
async function restoreCancelled(checkinId, checkinMethod) {
...
async function findTraining(trainingId) {
  ...
  if (!training) throw new Error("SQLに行事が登録されていません。");
  return training;
}
...
async function registerNewPerson(trainingId, data, method = CHECKIN_METHODS.WEB_SEARCH) {
  ...
  try {
    ...
    if (await restoreCancelled(checkinId, method)) {
      return {
        ok: true,
        message: "受付完了",
        companyName: data.companyName,
...
async function registerPlanned(trainingId, plannedId, method = CHECKIN_METHODS.PLANNED_QR) {
  ...
  if (!planned) throw new Error("この行事の有効な事前登録QRではありません。");
...
async function registerCompany(trainingId, memberNo, method) {
  ...
  try {
    ...
    if (!duplicateError(error)) throw error;
    if (await restoreCancelled(`${trainingId}:COMPANY:${memberNo}`, method)) {
...
async function registerPerson(trainingId, personalId, method) {
  ...
  try {
    ...
    if (!duplicateError(error)) throw error;
    if (await restoreCancelled(`${trainingId}:PERSONAL:${personalId}`, method)) {
...
export async function registerSqlQrCheckin(trainingId, rawCode, method = CHECKIN_METHODS.DIRECT_QR) {
  ...
  if (code.startsWith("MEMBER:")) {
    if (!isCompanyUnit(training)) throw new Error("この行事は個人単位受付です。個人QRを読み取ってください。");
    return registerCompany(trainingId, code.slice(7).trim(), method);
  }
  if (code.startsWith("PERSONAL:")) {
    if (isCompanyUnit(training)) throw new Error("この行事は会社単位受付です。会社QRを読み取ってください。");
